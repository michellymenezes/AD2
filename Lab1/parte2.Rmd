...
Para cada curso, foi calculada a mediana sobre todas as médias de disciplinas dos alunos, desconsiderando valores vazios. Abaixo temos dois gráficos que, de maneira ordenada, exibe os quatro melhores e piores piores cursos com relação a médias em disciplinas. 

```{r warning=FALSE}
cursos = cursos[!is.na(cursos$Media_Disciplina),]
cursos_mediana = aggregate(cursos$Media_Disciplina, list(cursos$Cod_Curso, cursos$Nome_Curso), FUN = median)

cursos_mediana = cursos_mediana[ order( cursos_mediana$x),]

ggplot(cursos_mediana[1:4,], aes( y = x, x = reorder(Group.2, -x))) + geom_bar(stat="identity", color = "#2171b5", fill = "#c6dbef") +  geom_text(aes(label=x),size = 4) + coord_flip() + labs(title = "Cursos com piores notas", x = "Curso", y = "Mediana")

ggplot(cursos_mediana[92:95,], aes( y = x, x = reorder(Group.2, -x))) + geom_bar(stat="identity", color = "#2171b5", fill = "#c6dbef") +  geom_text(aes(label=x),size = 4) + coord_flip()  + labs(title = "Cursos com melhores notas", x = "Curso", y = "Mediana")
```
...
ggplot(disc.calc, aes(Media_Disciplina.x, Media_Disciplina.y)) + geom_point(alpha = 0.5, color = "#7a0177") + labs(title = "Relação de notas Cálculo I x Matemática Discreta", x = "Notas de Cálculo I", y = "Notas de Matemática Discreta")
```

Apesar de um pouco disperso, no gráfico acima os pontos se desenham de maneira em é possóvel observar uma tendência. Desconsiderando outliers, vemos que a dendência é de que as notas de ambas as disciplinas geralmente crescem juntas. Esse é um indicativo de que há correlação entre as duas variáveis. Abaixo temos o valor da correlação calculado:
