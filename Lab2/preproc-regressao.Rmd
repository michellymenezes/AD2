...
library(readr)
library(dplyr)
graduados <- read_csv("../../../Downloads/graduados_treino_avaliacao.csv", col_types = cols(ALU_NOVAMATRICULA = col_character())) %>%
...
graduados.cra <- graduados.clean %>%
  ...
  mutate(cra.contrib = MAT_MEDIA_FINAL*ALU_PERIODO_EVASAO) %>%
  summarise(cra = sum(cra.contrib)/sum(ALU_PERIODO_EVASAO))
...
MAT_MEDIA_FINALhead(graduados.model.input)
